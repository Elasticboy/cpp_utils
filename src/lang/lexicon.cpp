...
const std::string lexicon::item(const std::string& key, const std::string& arg)
{
    ...
    if (!std::regex_search(str, match, r)) {
        ...
        log.Warning( "Lang::getText(), No $1 found for the key : \"" + key + "\". Argument \"" + arg + "\" is useless !");
        log.Warning("Lang::getText(), Consider using Lang::GetText(key) instead of Lang::GetText(key, arg)");
    }
